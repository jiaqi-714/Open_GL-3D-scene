...
namespace {
    ...
    void update_cam_pos(camera_t &cam, GLFWwindow *win, float dt, glm::vec3 moving_light_pos) {
...
camera_t make_camera(glm::vec3 pos, glm::vec3 target, bool FPSMode, int id) {
    ...
    return { pos, yaw, pitch, FPSMode, id};
}
...
void update_camera(camera_t &cam, GLFWwindow *window, float dt, glm::vec3 moving_light_pos) {
    update_cam_angles(cam, window, dt);
    update_cam_pos(cam, window, dt, moving_light_pos);
}
