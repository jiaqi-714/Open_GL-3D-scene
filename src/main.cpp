...
int main() {
    ...
    camera_t current_camera = scene.cam1;
    
    while (!glfwWindowShouldClose(win)) {
        float dt = time_delta();
        current_camera = switch_camera(win, scene, current_camera);
        animate(animator, scene, time_now());
        update_camera(current_camera, win, dt, scene.moving_light.pos);

        update_light(scene, win, dt);
        render(renderer, scene, current_camera);
