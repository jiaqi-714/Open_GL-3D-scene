...
renderer_t make_renderer(const glm::mat4 &projection) {
    ...
    GLint light2_pos_loc = chicken3421::get_uniform_location(render_program, "u_light2_pos");
    GLint light2_color_loc = chicken3421::get_uniform_location(render_program, "u_light2_color");

    GLint sun_direction_loc = chicken3421::get_uniform_location(render_program, "sun_light_dir");
    GLint sun_color_loc = chicken3421::get_uniform_location(render_program, "sun_light_color");
    ...
    return { projection,
             render_program,
             p_loc, v_loc, m_loc,
             light_pos_loc, light_color_loc,
             light2_pos_loc, light2_color_loc,
             sun_direction_loc, sun_color_loc,
             diff_map_loc, spec_map_loc,
             diffuse_loc, emissive_loc
    };
}
...
// TODO: add more here!
void render(const renderer_t &renderer, const scene_t &scene, camera_t used_camera) {
    ...
    glm::mat4 v = get_view(used_camera);
    ...
    // set up the light2 lighting
    // std::cout << scene.moving_light.pos.x << " " << scene.moving_light.pos.y << " " << scene.moving_light.pos.z << "\n";
    glUniform3fv(renderer.light2_pos_loc, 1, glm::value_ptr(scene.moving_light.pos));
    glUniform3fv(renderer.light2_color_loc, 1, glm::value_ptr(scene.point_light2.color));

    // set up the sun lighting
    glUniform3fv(renderer.sun_direction_loc, 1, glm::value_ptr(scene.sun_light.direction));
    glUniform3fv(renderer.sun_color_loc, 1, glm::value_ptr(scene.sun_light.color));
